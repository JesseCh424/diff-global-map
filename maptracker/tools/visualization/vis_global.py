...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help='Fix plotting bounds to these values (applies to both pred and gt).'
    )
    parser.add_argument(
        '--bounds_pkl',
        nargs='+',
        default=None,
        help='Optional aggregated scene pickle(s) to derive shared plotting bounds automatically.'
    )
...
def compute_bounds_from_pkls(pkl_paths):
    all_points = []
    for path in pkl_paths:
        if path is None or not os.path.exists(path):
            print(f"[warn] bounds_pkl not found: {path}")
            continue
        try:
            data = mmcv.load(path)
            ...
            continue
        for label in (0, 1, 2):
            for arr in data.get(label, []):
                arr_np = np.asarray(arr, dtype=np.float32)
                if arr_np.size:
                    all_points.append(arr_np)
        for entry in data.get("car_trajectory", []):
            center = np.asarray(entry.get("center", []), dtype=np.float32)
            if center.size == 2:
                all_points.append(center.reshape(1, 2))
    if not all_points:
        return None
...
def vis_pred_data(scene_name="", pred_results=None, origin=None, roi_size=None, args=None):
    ...
    if args.manual_bounds:
        x_min, x_max, y_min, y_max = args.manual_bounds
...
def main():
    ...
    if args.manual_bounds is not None:
        args.manual_bounds = tuple(args.manual_bounds)
    ...
    if args.manual_bounds is None and args.bounds_pkl:
        auto_bounds = compute_bounds_from_pkls(args.bounds_pkl)
        if auto_bounds is not None:
            args.manual_bounds = auto_bounds
