...
class AV2MapExtractor(object):
            ...
            for key, value in tmp.items():
                ...
                for idx, pred in enumerate(value['predecessors']):
                    if pred in tmp.keys():
                        ...
                        if not (pred_pt[0] == start_pt[0] and pred_pt[1] == start_pt[1] and pred_pt[2] == start_pt[2]):
                ...
                for idx, succ in enumerate(value['successors']):
                    if succ in tmp.keys():
                        ...
                        if not (end_pt[0] == succ_pt[0] and end_pt[1] == succ_pt[1] and end_pt[2] == succ_pt[2]):
    ...
    def proc_line(self, line, ego_SE3_city):
        ...
        line = LineString(new_line_pts_egoframe[:, :3])
        return line
    ...
    def extract_local_divider(self, nearby_dividers, ego_SE3_city, patch_box, patch_angle, patch_size):
        ...
        poly_centerlines = [
            line.buffer(0.1, cap_style=CAP_STYLE.flat, join_style=JOIN_STYLE.mitre) for line in centerlines
        ]
            ...
            if polygon.is_valid:
                ...
                if not new_polygon.is_empty:
                    if new_polygon.geom_type is 'Polygon':
                        ...
                        raise ValueError(f'{new_polygon.geom_type} is not valid')
                    if new_polygon.geom_type is 'Polygon':
    ...
    def get_map_geom(self, log_id: str, e2g_translation: NDArray, e2g_rotation: NDArray, polygon_ped=True):
