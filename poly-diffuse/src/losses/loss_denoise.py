...
class EDMLoss:
    ...
    def __call__(self, net, net_guide, images, z_init=None, **model_kwargs):
        ...
        # Choose the state to corrupt: mixed-init if provided, otherwise y
        x0 = z_init if z_init is not None else y
        n = torch.randn_like(x0) * sigma[..., None] * sigma_guide[:, :, None, :]
        noised_y = x0 + n
