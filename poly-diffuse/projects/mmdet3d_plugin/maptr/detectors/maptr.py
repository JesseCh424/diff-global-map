...
class MapTR(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        if img is not None:
            ...
            # When batch size == 1 and num_cams may also be 1, squeezing all
            # singleton dims would drop the camera dimension and produce
            # a 3D CHW tensor, which breaks the backbone expecting NCHW.
            # Only squeeze the batch dimension to keep (N_cams, C, H, W).
        ...
        for name, param in state_dict.items():
            ...
            if included:
                if name in own_state:
                    try:
                        # Shape-safe copy: only load when shapes match; otherwise try partial load on leading dim
                        if tuple(own_state[name].shape) == tuple(param.shape):
                            own_state[name].copy_(param)
                            loaded += 1
                            loaded_names.append(name)
                        else:
                            # Common mismatch: level_embeds or cams_embeds length differs (e.g., 6 vs 1).
                            os = own_state[name]
                            ps = param
                            if os.ndim == ps.ndim and os.shape[-1] == ps.shape[-1] and os.shape[0] <= ps.shape[0]:
                                # Copy prefix along leading dim
                                os_n = os.shape[0]
                                os[:os_n].copy_(ps[:os_n])
                                loaded += 1
                                loaded_names.append(name + "[:prefix]")
                            else:
                                # Skip incompatible tensors silently; keep random init
                                pass
                    except Exception:
                        # Any copy failure: skip and continue
                        pass
