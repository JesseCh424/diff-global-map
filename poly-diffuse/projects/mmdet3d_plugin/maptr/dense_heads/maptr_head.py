...
class MapTRHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.coords_embed = linear(2*self.input_dim, self.embed_dims)
        self.time_embed = nn.Sequential(
            linear(self.input_dim, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        )
        self.vertex_input_fc_noise = nn.Linear(3*self.embed_dims, self.embed_dims)
        self.class_embedding = nn.Embedding(3, self.embed_dims)

        self._init_layers()

        # (Reverted) No ProposalEncoder fusion in official-aligned head
    ...
    #TODO: understand the logics of the maptr head. Think about the migerating process..
    # @auto_fp16(apply_to=('mlvl_feats'))
    @force_fp32(apply_to=('mlvl_feats', 'prev_bev'))
    def forward(self, query_coords, timestep, poly_class, poly_mask, mlvl_feats, img_metas,
            prev_bev=None, only_bev=False, use_cached_feat=False, cache_image_feat=False):
        """Forward function.
        Args:
        ...
        assert self.query_embed_type == 'instance_pts'
        num_instance = query_coords.shape[1]

        pts_embeds = self.pts_embedding.weight.unsqueeze(0)
        instance_embeds = self.instance_embedding.weight.unsqueeze(1)
        object_query_embeds = (pts_embeds + instance_embeds[:num_instance]).flatten(0, 1).to(dtype)
        bev_queries = self.bev_embedding.weight.to(dtype)

        vert_coords = rearrange(query_coords, 'b n1 n2 c -> b (n1 n2) c')
        raw_vert_coords = (vert_coords + 1) / 2
        vert_coords = timestep_embedding((vert_coords.flatten() + 1) * 30, self.input_dim).view(bs, -1, 2, self.input_dim)
        vert_coords = torch.cat([vert_coords[:, :, 0, :], vert_coords[:, :, 1, :]], dim=-1)
        vert_coords = self.coords_embed(vert_coords)
        vert_query_embeds = repeat(object_query_embeds, 'n1 n2 -> b n1 n2', b=bs)

        poly_class_emb = self.class_embedding(poly_class)
        poly_class_emb = repeat(poly_class_emb, 'b n1 c -> b n1 n2 c', n2=self.num_pts_per_vec)
        poly_class_emb = rearrange(poly_class_emb, 'b n1 n2 c -> b (n1 n2) c')

        vert_encodings = torch.cat([vert_coords, vert_query_embeds, poly_class_emb], dim=-1)
        vert_encodings = self.vertex_input_fc_noise(vert_encodings)

        # (Reverted) No proposal fusion in official-aligned head

        bev_mask = torch.zeros((bs, self.bev_h, self.bev_w),
                               device=bev_queries.device).to(dtype)
        bev_pos = self.positional_encoding(bev_mask).to(dtype)

        poly_mask = rearrange(poly_mask, 'b n1 n2 -> b (n1 n2)')
        ...
        outputs_pts_coords = outputs_pts_coords * 2 - 1
        
        outs = {
            'bev_embed': bev_embed,
            'all_bbox_preds': outputs_coords,
            'final_results': outputs_pts_coords,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'enc_pts_preds': None
        }
    ...
    def transform_box(self, pts, y_first=False):
        ...
        """
        pts_reshape = pts.view(pts.shape[0], -1,
    ...
    def loss(self,
        ...
        """
        #TODO: understand the details here
